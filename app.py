...
<style>
    ...
    .stApp { background: var(--bg); font-family: 'Exo 2', sans-serif; }
    ...
    .hero-title { font-family:Rajdhani,sans-serif; font-size:2.2rem;
    ...
    .stTabs [data-baseweb="tab"] { font-family:Rajdhani,sans-serif!important;
        font-size:1rem!important; letter-spacing:1px!important; color:var(--muted)!important; }
...
PLOT_BG = dict(paper_bgcolor='#0a1628', plot_bgcolor='#050d1a',
...
def load_models():
    try:
        ...
        reg = load_model(f'{SAVE_DIR}/best_lstm_regression.keras')
        cls = load_model(f'{SAVE_DIR}/best_lstm_classification.keras')
...
def make_seq(features_scaled):
    seq = np.tile(features_scaled, (SEQUENCE_LENGTH, 1))
    return seq.reshape(1, SEQUENCE_LENGTH, len(FEATURES))
...
with st.sidebar:
    ...
    <div style='text-align:center;padding:1rem 0;'>
        ...
        <div style='font-family:Rajdhani,sans-serif;font-size:1.1rem;
                    color:#00d4ff;font-weight:700;letter-spacing:2px;'>BATTERY AI</div>
    ...
    page = st.radio("NAVIGATION",
        ["🏠 Dashboard","🔮 Live Prediction","📊 Model Performance",
...
if page == "🏠 Dashboard":
    ...
    prec = [97.94,85.71,98.87]
    rec  = [94.35,86.97,99.20]
    f1   = [96.11,86.34,99.03]
    for vals,name,color in [(prec,'Precision','#00d4ff'),(rec,'Recall','#00ff9d'),(f1,'F1-Score','#ff6b35')]:
        fig2.add_trace(go.Bar(name=name, x=classes, y=vals,
...
# ── LIVE PREDICTION ────────────────────────────────────────
    ...
    with tab1:
        ...
        if st.button("🔮 PREDICT NOW"):
            if not loaded:
                st.error("⚠️ Models not loaded! Mount Drive first.")
            else:
    ...
    with tab2:
        ...
        st.markdown(f"""<div class='icard'>
            📋 Required: <strong>Voltage, Current, Power, Temperature, CycleCount, State_encoded</strong>
            <br>State_encoded: 1=CHARGING, 0=DISCHARGING
        </div>""", unsafe_allow_html=True)
            ...
            if missing:
                ...
                if st.button("🔮 BATCH PREDICT") and loaded:
                    ...
                    for i in range(len(X)):
                        ...
                        probs = lstm_cls.predict(seq,verbose=0)[0]
                        cls_idx = int(np.argmax(probs))
                        results.append({'SoH':soh,'Usability':CLASS_NAMES[cls_idx]})
                    ...
                    fig_t = go.Figure(go.Scatter(
                        y=res_df['SoH'], mode='lines',
                        line=dict(color='#00d4ff',width=2), name='SoH',
...
# ── MODEL PERFORMANCE ──────────────────────────────────────
    ...
    fi_vals = [10.8637,0.1162,0.0118,-0.0074,-0.0158,-0.0448]
    fi_feat = ['Voltage','Power','Temperature','CycleCount','State','Current']
    fig_fi = go.Figure(go.Bar(x=fi_vals, y=fi_feat, orientation='h',
        marker_color=['#ff3366' if v==max(fi_vals) else '#00d4ff' for v in fi_vals],
        opacity=0.85, text=[f'{v:.4f}' for v in fi_vals], textposition='outside',
        textfont=dict(family='Share Tech Mono',size=10)))
    ...
    with c1:
        ...
        fig_o.update_layout(**PLOT_BG, title='R² — No Overfitting ✅',
            height=280, yaxis=dict(range=[0,110],gridcolor='#1a3a5c',linecolor='#1a3a5c'),
            margin=dict(l=5,r=5,t=40,b=5))
...
# ── DATA ANALYSIS ──────────────────────────────────────────
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("<div class='sec'>◈ DATA PREPROCESSING SUMMARY</div>", unsafe_allow_html=True)
    st.dataframe(pd.DataFrame({
        'Step':['Raw Data','Remove Noise','Remove Non-Solar','Voltage Filter','Final'],
    ...
    with c1:
        ...
        fig_v.update_layout(**PLOT_BG, title='Voltage (6.0V–8.2V)',
            height=280, xaxis_title='Voltage (V)', yaxis_title='Count',
            margin=dict(l=5,r=5,t=40,b=5))
        st.plotly_chart(fig_v, use_container_width=True)

    with c2:
        st.markdown("<div class='sec'>◈ FEATURE CORRELATION WITH SoH</div>", unsafe_allow_html=True)
        corr_v = [1.000,0.164,0.123,-0.454,-0.462,-0.490]
        corr_f = ['Voltage','CycleCount','Temperature','Power','State','Current']
        fig_c = go.Figure(go.Bar(
            x=corr_v, y=corr_f, orientation='h',
        ...
        fig_c.update_layout(**PLOT_BG, title='Correlation with SoH',
            height=280, xaxis=dict(range=[-0.6,1.2],gridcolor='#1a3a5c',linecolor='#1a3a5c'),
            margin=dict(l=5,r=5,t=40,b=5))
...
# ── ABOUT ──────────────────────────────────────────────────
    ...
    techs = [("🐍","Python 3.10","Core"),("🧠","TensorFlow 2.19","Deep Learning"),
