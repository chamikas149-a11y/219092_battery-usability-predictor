...
with st.sidebar:
    ...
    </div><hr style='border-color:#1a3a5c;'>
    """, unsafe_allow_html=True)

    page = st.radio("NAVIGATION",[
        "🏠 Dashboard","🔮 Live Prediction",
        "📊 Model Performance","🔍 Data Analysis","ℹ️ About"])

    st.markdown(f"""
    <hr style='border-color:#1a3a5c;'>
    <div style='font-family:Share Tech Mono,monospace;font-size:0.7rem;color:#7ba7cc;padding:0.5rem;'>
        <div style='color:#00d4ff;margin-bottom:0.4rem;'>MODEL STATUS</div>
        <div>⚡ LSTM Regression</div><div>⚡ LSTM Classification</div>
...
# ── LIVE PREDICTION ────────────────────────────────────────
    ...
    with tab2:
        ...
        uploaded = st.file_uploader("DROP CSV FILE", type=['csv'], key="csv_upload")
        if uploaded:
            ...
            if missing:
                ...
                if st.button("🔮 BATCH PREDICT", key="batch_predict_btn") and loaded:
                    results = []
                    bar = st.progress(0)
